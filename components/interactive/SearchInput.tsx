...
const SearchInput = ({
  ...
  const onSelect = (selected: any) => {
    setSearchText(
      options.filter((option) => option.title === selected)[0]?.title
    );
    setSelectedItem(options.filter((option) => option.title === selected)[0]);
  ...
  /* debounce하고 싶었는데..*/
  //const debouncedCheckNoResult = useCallback(debounce(checkNoResult, 100), []);
