...
const PopularEmdrTable = ({ data, className }: Props) => {
  const columns = ["순위", "방문지", "평균 체류 시간"];
      ...
      {data &&
        data.map((row, index) => (
          <TableBody key={index}>
            ...
            <TableCell>{makeEmdrName(row.emdName, row.riName)}</TableCell>
            <TableCell>
...
export default PopularEmdrTable;
